...
I used Principal Component Analysis(PCA) to reduce the dimemtionality by decomposing and transforming the features into principal components which accounted for about 98.61% of the variables.
...
finally, I used the available data to answer the research question and offered helpful insight to the business strategy that will not only satisfy and retain customers but also help to minimize waste by allocating the right number of inventory to the right shop. 
